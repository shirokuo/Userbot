...
async def spotify_dl(e):
    ...
    if not args:
        if reply and reply.text:
            ...
            return await e.eor("Give a Song name as well;", time=5)
    ...
    files = list_dir(dls)
    if not files:
        return await eris.eor(f"**No result for:**  `{args}`", time=30)
